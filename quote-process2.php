<?php
	...
	if (!$errormsg){
		...
		if(!$mgsetemple) {
			...
			$body_message = file_get_contents('mgsc-email-template/mgsc-email-template-quote2.php');
			$mgsemailshorttag = array("[mgs-sender-ip]", "[mgs-sender-name]", "[mgs-sender-email]", "[mgs-sender-phone]", "[mgs-sender-service]", "[mgs-sender-project-budget]", "[mgs-sender-priority]", "[mgs-sender-message]");
			$mgsemailshorttagvalue   = array(get_client_ip(), $fname, $email, $phone, $service, $probudget, $priority, $message);
