<?php
	...
	if (!$errormsg){
		...
		if($_POST["subject"])
			...
			$mail->Subject = "New Clean Form demo Job Application Submitted";
		...
		if(!$mgsetemple) {
			...
			$body_message = file_get_contents('mgsc-email-template/mgsc-email-template-job.php');
			$mgsemailshorttag = array("[mgs-sender-ip]", "[mgs-sender-name]", "[mgs-sender-email]", "[mgs-sender-phone]", "[mgs-sender-address]", "[mgs-sender-jobtitle]", "[mgs-sender-message]");
			$mgsemailshorttagvalue   = array(get_client_ip(), $fname, $email, $phone, $address, $jobtitle, $message);
