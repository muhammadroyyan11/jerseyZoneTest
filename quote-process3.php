<?php
	...
	if (empty($_POST["sittingroom"])) {
		$errormsg .= "Sitting/Conference Room required. ";
	} else {
	...
	if (!$errormsg){
		...
		if(!$mgsetemple) {
			...
			$body_message .= "Sitting/Conference Room: " . $sittingroom ."<br>";
			...
			$body_message = file_get_contents('mgsc-email-template/mgsc-email-template-quote3.php');
			$mgsemailshorttag = array("[mgs-sender-ip]", "[mgs-sender-selected-property-type]", "[mgs-sender-required-cleaning-type]", "[mgs-sender-property-size]", "[mgs-sender-property-bedrooms]", "[mgs-sender-property-bathrooms]", "[mgs-sender-property-sittingroom]", "[mgs-sender-property-kitchen]", "[mgs-sender-property-kitchenitems]", "[mgs-sender-preferreddate]", "[mgs-sender-property-address]", "[mgs-sender-name]", "[mgs-sender-email]", "[mgs-sender-phone]", "[mgs-sender-message]");
			$mgsemailshorttagvalue   = array(get_client_ip(), $servicetype, $cleaningtype, $propertysize, $bedrooms, $bathrooms, $sittingroom, $kitchen, $kitchenitems, $preferreddate, $propertyaddress, $fname, $email, $phone, $message);
